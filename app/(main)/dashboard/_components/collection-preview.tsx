...
const CollectionPreview = (props:any) => {
  ...
  if(isCreateNew) {
    return <button 
        ...
        <div className={`relative h-full rounded-lg p-6 shadow-md hover:shadow-lg
            transition-all flex flex-col items-center justify-center gap-4 ${colorSchemes["createCollection"].tab}`} 
        >
            <div className="h-12 w-12 rounded-full bg-gray-200 group-hover:bg-gray-300
  ...
  return (
    <div>
      ...
      >
        <FolderTab colorClass={
            colorSchemes[isUnorganized ? "unorganized" : "collection"].tab} 
        />
        <div className={`relative h-full rounded-lg p-6 shadow-md hover:shadow-lg
            transition-all ${colorSchemes[isUnorganized ? "unorganized" : "collection"].bg}`} 
        >
            ...
            <div className="space-y-2">
                ...
                <div className="space-y-2 mt-4">
                    {entries.length > 0 ?(
                        entries.slice(0,2).map((entry:any) => <EntryPreview key={entry.id} entry={entry} />)) 
